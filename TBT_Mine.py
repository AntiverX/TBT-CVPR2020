...
class TBTPuls():
    ...
    def identify_target_neural(self):
        ...
        for name, module in self.net_for_trigger_insert.named_modules():
            ...
            # if name == "1.layer4.1.conv2":
            #     v, i = module.weight.grad.detach().abs().sum((1,2,3)).sort(descending=True)
            #     print(i)

        if opt.only_zero_affected_neural == 1:
            logger.info("only consider affects to acc.")
            exit()
            # target_neural_index = torch.tensor(zero_affected_neural_neural_index[:self.wb], dtype=int).cuda()
        else:
            logger.info("consider affects to acc and gradient.")
            target_neural_index = torch.tensor(targeted_neural[:self.wb], dtype=int).cuda()
    ...
    def insert_trojan(self):
        ...
        gradient_mask2[self.target_neural_index] = 1.0
